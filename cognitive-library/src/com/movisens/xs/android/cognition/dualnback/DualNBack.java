...
/**
 * Code adapted from: https://github.com/jtolds/dualnback
 */
...
public class DualNBack extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		((TextView)findViewById(R.id.n_value)).setText("N = " + m_nback);

		((Button) findViewById(R.id.auditory_button))
		...
		((Button) findViewById(R.id.visual_button))
				.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						visual();
					}
		...
		((Button) findViewById(R.id.both_button))
				.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v) {
						auditory();
						visual();
	...
	public void stopGame() {
		...
		((TextView) findViewById(R.id.status))
				.setText(getString(R.string.dualnback_status_stopped));
	...
	public void gameTick() {
		...
		if (m_positionHistory.length <= m_nback) {
			switch (m_nback - m_positionHistory.length) {
			case 0:
			...
			case 2:
				((TextView) findViewById(R.id.status))
						.setText("On your mark...");
				break;
			default:
				((TextView) findViewById(R.id.status)).setText("Wait...");
				break;
			}
			TextView[] new_positionHistory = new TextView[m_positionHistory.length + 1];
			...
			if (m_letterHistory[0] == m_letterHistory[m_nback]) {
			...
			if (m_positionHistory[0] == m_positionHistory[m_nback]) {
				if (m_visualClicked) {
					status += " - GOT POSITION!";
					m_successes += 1;
				} else {
		...
		if (m_positionHistory.length > m_nback
			...
			m_positionHistory[0] = m_positionHistory[m_nback];
		} else {
			m_positionHistory[0] = m_positions[GENERATOR
					.nextInt(m_positions.length)];
		}
		if (m_positionHistory.length > m_nback
				&& GENERATOR.nextDouble() < CHOOSE_MATCH_PROB) {
			m_letterHistory[0] = m_letterHistory[m_nback];
		} else {
			m_letterHistory[0] = m_letters[GENERATOR.nextInt(m_letters.length)];
		}

		((TextView) findViewById(R.id.skill)).setText("" + m_successes + "/"
				+ m_trials);
